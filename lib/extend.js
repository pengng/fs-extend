...
extend.listFiles = function (fullpath, callback) {
  fullpath = /^\//.test(fullpath) ? fullpath : path.resolve(process.cwd(), fullpath)
  const done = function (func) {
    return function (err) {
      if (err) {
  ...
  const readdirCallback = function (files) {
    ...
    each(arr, extend.listFiles, done(eachCallback))
  }
  const statCallback = function (stat) {
    if (stat.isDirectory()) {
      return fs.readdir(fullpath, done(readdirCallback))
...
extend.listFilesSync = function (fullpath) {
  fullpath = /^\//.test(fullpath) ? fullpath : path.resolve(process.cwd(), fullpath)
  const stat = fs.lstatSync(fullpath)
  if (stat.isDirectory()) {
    const files = fs.readdirSync(fullpath)
    ...
    let arr = []
    for (let i = 0, len = files.length; i < len; i++) {
      arr = arr.concat(extend.listFilesSync(path.resolve(fullpath, files[i])))
    }
    return arr
  }
  return [fullpath]
...
extend.empty = function (fullpath, callback) {
  fullpath = /^\//.test(fullpath) ? fullpath : path.resolve(process.cwd(), fullpath)
  const done = function (func) {
    return function (err) {
      if (err) {
  ...
  const eachCallback = function (result) {
    const isEmpty = result.every(function (item) {
      return item[0]
    })
  ...
  const readdirCallback = function (files) {
    ...
    each(arr, extend.empty, done(eachCallback))
  }
  const statCallback = function (stat) {
    if (stat.isDirectory()) {
      return fs.readdir(fullpath, done(readdirCallback))
...
extend.emptySync = function (fullpath) {
  fullpath = /^\//.test(fullpath) ? fullpath : path.resolve(process.cwd(), fullpath)
  const stat = fs.lstatSync(fullpath)
  if (stat.isDirectory()) {
    const files = fs.readdirSync(fullpath)
    ...
    for (let i = 0, len = files.length; i < len; i++) {
      const isEmpty = extend.emptySync(path.resolve(fullpath, files[i]))
      if (!isEmpty) {
        return false
      }
    }
